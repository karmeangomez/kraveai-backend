...
async function crearCuentaInstagram() {
    ...
    try {
        if (!proxyObj) {
            logger.warn('⚠️ Sin proxy disponible, intentando con IP local');
        } else {
        ...
        if (!emailInput) {
            throw new Error('No se encontró el campo emailOrPhone');
        }
        ...
        if (!signUpButton) {
            throw new Error('No se encontró el botón de registro');
        }
...
// 🔁 EXPORTACIÓN COMPATIBLE CON run.js
